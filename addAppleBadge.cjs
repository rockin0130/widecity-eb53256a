...
// Add Apple Badge component after GoogleBadge
...
// Show Apple badge for apple- prefixed events, Google badge for others
content = content.replace(
  '{item.type === "gcal" && <GoogleBadge />}\n                {item.type === "gcal" && <GoogleBadge />}',
  '{item.type === "gcal" && (item.id?.startsWith("apple-") ? <AppleBadge /> : <GoogleBadge />)}\n                {item.type === "gcal" && (item.id?.startsWith("apple-") ? <AppleBadge /> : <GoogleBadge />)}'
);

// Fix both occurrences individually
content = content.replace(
