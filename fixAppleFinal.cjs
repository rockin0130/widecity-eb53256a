...
// Replace entire Apple Calendar block with Google-compatible format
const oldBlock = `        if (Array.isArray(appleCalendarEvents)) { appleCalendarEvents.forEach((ae) => {
...
const newBlock = `        if (Array.isArray(appleCalendarEvents)) {
        appleCalendarEvents.forEach((ae) => {
          ...
          items.push({
            ...
            groupId: null,
            raw: { id: ae.id, title: ae.title, description: null, start: appleStart.toISOString(), end: appleEnd.toISOString(), allDay: ae.allDay, location: ae.location || null, htmlLink: "", calendarColor: ae.calendarColor || null } as GoogleCalendarEvent,
          });
        });
      }`;
